                            ...
                            <div class="row">
                                <div class="col-sm-12 b-r">
                                    ...
                                    <form role="form" method="post" action="{{route('messages.store')}}">
                                        ...
                                        @endif
                                        @php
                                        $edit['recipients'] = (!is_null($edit['recipients']))?(array)$edit['recipients']:[];
                                        @endphp
                                            ...
                                            <select class="form-control" name="recipients" >
                                                ...
                                                @if(count($recipients)>0)
                                                    @foreach($recipients as $k=>$v)
                                                        <option value="{{$v['id']}}" {{in_array($v['id'],$edit['recipients'])?'selected':''}}>{{$v['name']}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="template_source">
                                                ...
                                                <option value="freeform" {{($edit['template_source']=="freeform")?'selected':''}}>Free Form</option>
                                                <option value="library" {{($edit['template_source']=="library")?'selected':''}}>Template Library</option>
                                                <option value="ai_assistence" {{($edit['template_source']=="ai_assistence")?'selected':''}}>AI Assisted</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Subject</label>
                                            <input type="text" class="form-control" name="subject" value="{{old('subject',$edit['subject'])}}"/>
                                        </div>
                                        <div class="form-group">
                                            <label>Message</label>
                                            <textarea class="form-control" rows="2" name="message">{{old('message',$edit['message'])}}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label>Delivery Type</label>
                                            <br>
                                            <label>
                                                <input type="radio" class=" delivery_type" name="delivery_type" {{($edit['delivery_type']=="death")?'checked':''}} onclick="deliveryType('death');" value="death"> Death
                                            </label>
                                            <label>
                                                <input type="radio" class=" delivery_type" name="delivery_type" {{($edit['delivery_type']=="schedule")?'checked':''}}  onclick="deliveryType('schedule');" value="schedule"> Schedule
                                            </label>
                                        </div>
                                        <div class="form-group delivery_date d-none">
                                            <label>Date of delivery</label>
                                            <input type="date" class="form-control" name="delivery_date" value="{{old('delivery_date',$edit['delivery_date'])}}"/>
                                        </div>
                                        <div>
