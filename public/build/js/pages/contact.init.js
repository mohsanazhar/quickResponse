        ...
        columns: [
            {
                ...
                data: function (row) {
                    return gridjs.html(
						...
						<div class="avatar-xs rounded-circle"><img src="' +
                            row.user.img +
                            '" alt="" class="img-fluid rounded-circle d-block user-img"></div>\
						</div>\
						<div class="flex-grow-1">\
						<h6 class="fw-medium mb-1 user-name">' +
                            row.user.name +
                            "</h6>\
						</div>\
						</div>"
            ...
            },
            {
                ...
                data: function (row) {
                    return gridjs.html(
                        ...
                        <div><a href="#addContactModal" data-bs-toggle="modal" onClick="editMemberList(' +
                            row.id +
                            ')"  class="text-muted px-1 d-block"><i class="bi bi-pencil-fill"></i></a></div>\
                        <div><a href="#removeContactModal" data-bs-toggle="modal" class="text-muted px-1 d-block" onClick="removeItem(' +
                            row.id +
            ...
            function (event) {
                if (!form.checkValidity()) {
                    ...
                    if (
                        ...
                        var getEditid = 0;
                        getEditid =
                            document.getElementById("contactid-input").value;
...
Array.from(document.querySelectorAll(".addContact-modal")).forEach(function (
    ...
    elem.addEventListener("click", function (event) {
        ...
        document.getElementById("contact-img").src =
            "build/images/users/user-dummy-img.jpg";
        document
...
function overviewList() {
    ...
    var userCountry = rowElem.querySelector(
        "[data-column-id='country']"
    ).innerHTML;
    var userLabel = rowElem.querySelector(
        "[data-column-id='label'] .badge"
    ).innerHTML;

    document.querySelector("#viewContactoffcanvas .overview-userimg").src =
        userImg;
    document
...
searchProductList.addEventListener("keyup", function () {
    ...
    function filterItems(arr, query) {
        return arr.filter(function (el) {
            return (
                el.user.name.toLowerCase().indexOf(query.toLowerCase()) !==
                    -1 ||
                el.email.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
                el.phone.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
                el.country.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
...
Array.from(document.querySelectorAll(".contact-menu-list a")).forEach(function (
    menuTab
) {
        ...
        if (menuTab.querySelector(".menu-list-link").hasAttribute("data-tab")) {
            ...
            if (tabname != "all") {
                var filterData = contactData.filter(
                    (list) => list.tab === tabname
                );
            } else {
                var filterData = contactData;
            }
        } else if (
            menuTab.querySelector(".menu-list-link").hasAttribute("data-label")
        ) {
            var tabname = menuTab
                .querySelector(".menu-list-link")
                .getAttribute("data-label");
            var filterData = contactData.filter(
                (list) => list.label === tabname
            );
        }

        var activeListTab = document.querySelector(
            ".contact-menu-list a.active"
        );
        if (activeListTab) {
